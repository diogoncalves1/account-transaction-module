...
class TransactionRepository implements RepositoryApiInterface
{
    ...
    public function update(Request $request, string $id)
    {
        try {
            return DB::transaction(function () use ($request, $id) {
                ...
                $user = $request->user();
                $sharedRole = $this->accountRepository->userSharedRole($transaction->account, 2);
    ...
    public function dataTable(Request $request)
    {
        ...
        $query =  Transaction::join("accounts", "accounts.id", "=", "transactions.account_id")
            ->join("account_users", "accounts.id", '=', 'account_users.account_id')
            ->leftJoin("users", "users.id", '=', 'transactions.user_id')
            ->leftJoin("categories", "categories.id", '=', 'transactions.category_id')
            ->join("currencies", "currencies.id", '=', 'accounts.currency_id')
            ->where("account_users.user_id", 2);
        ...
        $transactions = $query->offset($request->start)
            ...
            ->select("users.name as user", "accounts.balance", "accounts.id", "currencies.symbol as currency", "transactions.type", "transactions.*")
            ->get();
